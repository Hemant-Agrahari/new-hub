...
const Header = ({ handleSearch, searchQuery, setActive, active }: any) => {
  ...
  const tab = useAppSelector((state) => state.games.tab)
  const activeId = useAppSelector((state) => state.games.activeId)
  const divSettingTabAPI = useAppSelector(
    (state) => state.games.divSettingTabAPI,
  )
  ...
  useEffect(() => {
    ...
    dispatch(setTab(resparam as any))
  }, [activeId, divSettingTabAPI, route, tab])
